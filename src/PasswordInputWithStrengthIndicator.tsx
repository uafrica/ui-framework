...
function PasswordInputWithStrengthIndicator(props: IProps) {
  ...
  function calculateScore() {
    ...
    const lowercaseScore = shouldContainLowercase ? (/[a-z]/.test(password) ? 1 : 0) : 0;
    const uppercaseScore = shouldContainUppercase ? (/[A-Z]/.test(password) ? 1 : 0) : 0;
    const numbersScore = shouldContainNumbers ? (/\d/.test(password) ? 1 : 0) : 0;
    ...
    criteriaMet = lengthScore + lowercaseScore + uppercaseScore + numbersScore + specialCharsScore;
  ...
  function getUnmetCriteriaMessages() {
    let criteriaMessages = [
      ...
      { message: "Must contain a number", met: shouldContainNumbers && /\d/.test(password) },
      {
        message: "Can contain a special character",
        met: shouldContainSpecialCharacters && /[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/.test(password)
      }
    ];
  ...
  function render() {
    return (
      <div className="w-full">
        <label className="block font-semibold mb-1">{inputLabel ? inputLabel : "Password"}</label>
